...
public class AdminInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        if(token != null && jwtUtil.checkToken(token)){
            log.info("AdminInterceptor Use Token : {}", token);
            log.info("AdminInterceptor Role : {}", jwtUtil.getUserRole(token));
            if(jwtUtil.getUserRole(token).equals("ADMIN")) {
                return true;
            }
            else {
            ...
            log.info("AdminInterceptor InvalidToken : {}", token);
            response.setContentType("application/json");
            response.setCharacterEncoding("utf-8");
            temp.put("status", 401);
            temp.put("success", false);
